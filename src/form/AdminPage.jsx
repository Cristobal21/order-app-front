...
export const AdminPage = () => {
  ...
  return (
    <>
      {getInfo.length !== 0
        ? (
          <section className='print-content grid justify-center gap-5 mx-4 mt-4 mb-36 lg:mt-32 xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 sm:w-auto'>
            {getInfo.map((order) => (
              <section
                key={order._id}
              ...
              >
                <h2 className='flex flex-col justify-center text-center'>
                  ...
                  <button onClick={handleClickModal} id='modifyOrder' className='border rounded-md bg-slate-300 py-1 px-3 hover:opacity-70'>{order._id}</button>
                </h2>
                ...
                {/* <section className='w-full flex justify-center  '>
                  <button className='text-xl w-60 rounded-lg py-3 px-4 text-white active:scale-95 hover:opacity-70 active:opacity-100 bg-cyan-700'>Imprimir</button>
                </section> */}
              </section>
            ))}
